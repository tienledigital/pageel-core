 ...
 * 
 * Modal for creating a new collection with name and paths.
 */

import React, { useState } from 'react';
import { createCollection, useCollectionStore, Collection } from '../features/collections';
...
export const NewCollectionModal: React.FC<NewCollectionModalProps> = ({
  ...
  const { addCollection, workspace } = useCollectionStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (workspace?.collections.some(c => c.id === id)) {
      setError(t('collectionModal.error.duplicateName'));
      return;
    }

    // Create collection
    const collection = createCollection(id, name.trim(), postsPath.trim(), imagesPath.trim());
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 animate-fade-in">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-notion-border">
          <h2 className="text-lg font-semibold text-notion-text">{t('collectionModal.newTitle')}</h2>
          <button
            onClick={handleReset}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {t('collectionModal.createButton')}
            </button>
          </div>
        </form>
      </div>
